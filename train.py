...
from utils import build_optimizer, init_seed, init_logger, init_device, read_configuration, collate_fn_graph_text, \
    format_time
from module import GraphEncoder, GraphReconstructor, GraphPointer ,GATEncoder
...
def compute_kd_loss(node_embeddings, desc_embeddings, node_masks, kd_masks):
    assert node_embeddings.size() == desc_embeddings.size(), f"Shape mismatch: {node_embeddings.size()} vs {desc_embeddings.size()}"
...
def run_train_batch(config, batch, teacher, student, plm, reconstructor, copyer,
                    plm_optimizer, external_optimizer, device, gat_encoder):
    ...
    # print("before ",student_embeddings.shape)
    student_embeddings = gat_encoder(student_embeddings,edges)
    # print("after ",student_embeddings.shape)
...
def run_eval_batch(config, batch, teacher, student, plm, reconstructor, copyer, device, gat_encoder):
    nodes, edges, types, node_masks, kd_description, kd_description_masks, kd_positions, \
        recon_relations, recon_positions, recon_masks, gen_outputs, gen_masks, pointer, pointer_masks = batch
...
def train(config):
    ...
    gat_encoder = GATEncoder(config["embedding_size"],config["gnn_layers"], dropout_ratio=0.3)
    student.to(device)
    ...
    for p in gat_encoder.parameters():
        if p.requires_grad:
            external_parameters.append(p)
    ...
    for epoch_idx in range(config["start_epoch"], config["epochs"]):
        ...
        for batch_idx, batch in tqdm(enumerate(train_dataloader), total=len(train_dataloader), desc=f"Epoch {epoch_idx}", ncols=100):
            gen_loss, rec_loss, kd_loss, copy_loss = run_train_batch(config, batch, teacher, student, plm, reconstructor,
                                                                     copyer, plm_optimizer, external_optimizer, device, gat_encoder)
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(valid_dataloader, total=len(valid_dataloader), desc="Validation", ncols=100):
                gen_loss, rec_loss, kd_loss, copy_loss = run_eval_batch(config, batch, teacher, student, plm,
                                                                        reconstructor, copyer, device, gat_encoder)
...
def test(config):
    ...
    student.to(device)
    gat_encoder = GATEncoder(config["embedding_size"],config["gnn_layers"], dropout_ratio=0.3)
    gat_encoder.load_state_dict(torch.load(config["external_model"])["gat_encoder"])
    gat_encoder.to(device)
    logger.info("Build PLM Model.")
    ...
    # Wrap the DataLoader with tqdm for the testing loop
    with torch.no_grad():
        for batch in tqdm(test_dataloader, total=len(test_dataloader), desc="Testing", ncols=100):
            nodes, edges, types, node_masks, kd_description, kd_description_masks, kd_positions, \
                recon_relations, recon_positions, recon_masks, gen_outputs, gen_masks, pointer, pointer_masks = batch
